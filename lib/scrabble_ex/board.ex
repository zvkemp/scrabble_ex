defmodule ScrabbleEx.Board do
  ...
  defp parse_fragment(f), do: f # FIXME: match on length
  ...
  def word_maps(board) do
    ...
    starts = (0..max)
    |> Enum.filter(fn i -> is_binary(board.state[i]) end) # words may start here
